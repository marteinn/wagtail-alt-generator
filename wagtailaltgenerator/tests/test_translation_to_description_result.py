...
class TranslationToDescriptionResultTest(TestCase):
    ...
    def test_translation_mapping(self):
        ...
        self.assertEqual(
            translated_result.description, "Min titel"
        )
        self.assertEqual(
            translated_result.tags, ["katt", "hus", "sko"]
        )

    @mock.patch.object(
        GoogleTranslate,
        'translate',
        lambda *args, **kwargs: ['katt', 'hus', 'sko'],
    )
    ...
    def test_that_description_is_set_when_tags_are_none(self):
        ...
        self.assertEqual(
            translated_result.description, "A bridge"
        )
        self.assertEqual(
            translated_result.tags, []
        )
